  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        console.error('Konum, tarih ve not yüklenirken hata:', error);
      }
    };
  ...
  const handleSave = async () => {
    if (editFields.body.trim() || editFields.title.trim()) {
      ...
      try {
        ...
        await setDoc(docRef, { note: noteContent, dateRange: editFields.dateRange }, { merge: true });
            ...
            <div className="px-4 pt-4 pb-10 bg-white">
              ...
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mb-6 text-xs text-gray-500">
                <span className="flex items-center gap-1 leading-none">
                  ...
                  {formatDateForDisplay(entry.dateRange)}
                </span>
                <span className="flex items-center gap-1 leading-none">
                  <svg className="w-3.5 h-3.5 shrink-0 -translate-y-px" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
              ...
              >
                ...
                <div className="flex-1 px-5 py-4 overflow-hidden flex flex-col gap-3">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <input
                      type="date"
                      value={editFields.dateRange}
                      onChange={(e) => setEditFields({ ...editFields, dateRange: e.target.value })}
                      onPointerDownCapture={(e) => e.stopPropagation()}
                      className="rounded-xl px-4 py-2 text-sm text-gray-800 bg-white focus:outline-none leading-relaxed"
                      style={{ fontFamily: 'var(--font-comfortaa), sans-serif', border: '1.5px solid #E65100', boxShadow: 'none' }}
                    />
                  </div>

                  {/* Note Textarea */}
                  <div className="flex-1 flex flex-col gap-2 overflow-hidden">
                    <label className="text-xs font-semibold text-gray-600">Not</label>
                    <textarea
                      autoFocus
